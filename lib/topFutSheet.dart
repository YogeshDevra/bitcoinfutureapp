...
import 'futLoser.dart';
import 'futMap.dart';

class TopFutSheet extends StatefulWidget {
  const TopFutSheet({Key? key}) : super(key: key);
  @override
  _TopFutSheet createState() => _TopFutSheet();
}

class _TopFutSheet extends State<TopFutSheet> with SingleTickerProviderStateMixin{

  bool isLoading = false;
  late List<FutCrypto> futGainer = [];
                      ...
                      child: Row(
                        children: [
                          Padding(
                            ...
                            child: Text(FutLang.of(context).translate('top_gain'),textAlign: TextAlign.left,
                          ...
                          Padding(
                            ...
                            child: InkWell(
                              onTap: () {
                                Navigator.push(context, MaterialPageRoute(builder: (context) => const LoserFut()),);
                              }, // Image tapped
                              child: Container(
                                decoration: BoxDecoration(color: const Color(0xff2e3546),borderRadius: BorderRadius.circular(20)),
                                child: Padding(
                                  padding: const EdgeInsets.all(15),
                                  child: Text(FutLang.of(context).translate('lose'),textAlign: TextAlign.center,
  ...
  Widget topGainer(){
    var list = futGainer.where((element) => double.parse(element.diffRate!)>= 0).toList();
    ...
    return ListView.builder(
        ...
        itemBuilder: (BuildContext context, int i) {
          return InkWell(
            ...
            onTap: () {
              callCurrencyDetails(futGainer[i].name);
            },
          );
        });
  }
